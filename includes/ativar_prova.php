...
if (mysqli_num_rows($result_verificar) === 0) {
    $_SESSION['erro_prova'] = "Prova não encontrada ou você não tem permissão para ativá-la.";
...
// ✅ ATIVAR A PROVA
$sql_ativar = "UPDATE Provas SET ativa = 1 WHERE idProvas = ?";
$stmt_ativar = mysqli_prepare($conectar, $sql_ativar);
mysqli_stmt_bind_param($stmt_ativar, "i", $prova_id);

if (mysqli_stmt_execute($stmt_ativar)) {
    $_SESSION['sucesso_prova'] = "Prova ativada com sucesso! Agora os alunos podem visualizá-la.";
} else {
    $_SESSION['erro_prova'] = "Erro ao ativar a prova: " . mysqli_error($conectar);
}

mysqli_stmt_close($stmt_ativar);
mysqli_close($conectar);
